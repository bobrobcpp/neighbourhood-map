    ...
    $.getJSON(self.bigUrl, function(data) {
            ...
            data.response.group.results.forEach(function(loc) {
                ...
                self.locationList.push({
                    ...
                    bigPhoto: this.record.bigPhoto,
                    smallPhoto: this.record.smallPhoto,
                    rating: this.record.rating,
                    checkins: this.record.checkins,
                    infoContent: this.record.infoContent,
                    formattedAddress: [this.record.formattedAddress],
                    mark: this.record.mark
                });
            });
...
const placeListItem = function(data) {
    ...
    const theLatLng = new google.maps.LatLng(self.lat, self.lng);
    ...
    self.infoContent = function() {
        let contentString = "<div id='info-title'><h4>" + (self.name()).toString() + "</h4></div><div id='info-container'><br> <div id='info-window'><p><i class='fa fa-home info-icon' aria-hidden='true'> </i>";
